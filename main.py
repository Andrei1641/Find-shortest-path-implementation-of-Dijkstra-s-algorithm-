    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if event.key == pygame.K_k:
                if first_chosen_node and second_chosen_node:
                    typing_mode = 1


            if event.key == pygame.K_RETURN and typing_mode == 1:
                typing_mode = 2
            ...
            if typing_mode == 3:
                if event.key == pygame.K_RETURN:
                    print(first_chosen_node[1], int(towards_time), int(backwards_time))
                    second_chosen_node[0].add_path(first_chosen_node[0], int(towards_time), int(backwards_time))
                    typing_mode = 0


    for n, n_pos in pos_nodes.items():
        pygame.draw.circle(screen, C, n_pos, NODE_RADIUS)
        for _, next_n in n.paths:
            pygame.draw.line(screen, C, pos_nodes[n] , pos_nodes[next_n], 20)



    if first_chosen_node:
        pygame.draw.circle(screen, C[1] + 100, first_chosen_node[1], NODE_RADIUS)
    if second_chosen_node:
        pygame.draw.circle(screen, C[1] + 100, second_chosen_node[1], NODE_RADIUS)
